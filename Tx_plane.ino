...
void loop() {
  ...
  data.yval_l = map(analogRead(joyl_y), 1023, 0, 0, 255);
  ...
  if (radio.write(&data, sizeof(data_pack))) {
    ...
    lcd.setCursor(0, 0);
    lcd.print("T:" + String((data.yval_l / 255) * 100));
    //Serial.print("Th: " + String((data.yval_l / 255) * 100));

    lcd.setCursor(7, 0);                  //printing roll value on LCD
    if (analogRead(joyr_x) < 511) {
      lcd.print("Rl:L");
  ...
  lcd.setCursor(12, 0);                   //printing rudder value on LCD
    ...
    //Serial.print("Trim2:" + String(angl2));
  }
